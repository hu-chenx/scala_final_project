...
object TFIDF {
      ...
      def getTfidf(fbClient:DefaultFacebookClient,path : String, id:String,initialSpark : SparkContext)={
    val sc = initialSpark
...
//<------------------------------------------------------------------>
 ...
 def get_user_kw( word_outcome:String, sc:SparkContext,fbClient:DefaultFacebookClient,filePath:String) : Array[(String,String)] = {
  ...
  a.head.replaceAll(".txt","")
      ...
      //feature_word.foreach { x => println(x) }
   
       val id_word_rdd = feature_word.zip(file_path)
       
        val associative_user_rdd = id_word_rdd.filter{f => f._1.equals(word_outcome) }.toArray()
        
        associative_user_rdd
 }
